...
int swap(int *type, int idElementInGroup,
                ...
                int *idNewNode,
                CalendarQueue* q);
//Trả về 1 nghĩa là tham số cần truyền vẫn thêm vào Array Splay
//trả về 0 nghĩa tham số cần truyền KHÔNG thêm vào Array Splay 
...
int swap(int *type, int idElementInGroup,
    ...
    //middle:      3 vs 0
    //rightBound:  5 vs 0
    if(haveEmptySlot == 0)//Nếu không có ô nào trống
    {
        ...
        if(q->qsize == 0)//Neu calendar trống
        {
            return 1;//Thêm các tham số vào ngay Array Splay
        }
        else{
