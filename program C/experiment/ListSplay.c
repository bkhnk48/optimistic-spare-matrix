...
node* getNewNode(splay_tree *t, int type, int idElementInGroup,
                int portID, 
                int endTime){
  if(t->trash == NULL)
    return new_node(type, idElementInGroup, portID, endTime);
  node *temp = t->trash;
  t->trash = t->trash->left;
  temp->type = type;
  temp->idElementInGroup = idElementInGroup;
  temp->portID = portID;
...
void insert(splay_tree *t, int type, int idElementInGroup,
                int portID, 
                int endTime) {
  node *n = getNewNode(t, type, idElementInGroup, portID, endTime);
  node *y = NULL;
  node *temp = t->root;
  while(temp != NULL) {
    y = temp;
...
void delete(splay_tree *t, node *n) {
  ...
  splay_tree *left_subtree = new_splay_tree(1);
  left_subtree->root = t->root->left;
  if(left_subtree->root != NULL)
    left_subtree->root->parent = NULL;

  splay_tree *right_subtree = new_splay_tree(1);
