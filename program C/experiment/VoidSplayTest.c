...
int main(int argc, char** argv) 
{
    ...
    for (i = 0; i < numOfHosts; i++) //Only test first hosts in pod
    {
        ...
        insert(t, new_nodeS(A, i, 0, i));
        numOfFlows++;
    }

    NodeS * ev = removeFirst(t);
    while(currentTime <= endTime && ev->endTime != -1)
    {
        if(ev->endTime == currentTime)
        {
            ...
            if(ev->type == A)
            {
                ...
                insert(t, new_nodeS(A, i, 0, currentTime + T));
                generateEventB = actionA(T, currentTime, &bufferHosts[i]);
                if(generateEventB)
                    insert(t, new_nodeS(B, i, 0, currentTime +  defaultBias*13));
            }
            else if(ev->type == B)
            {
                generateEventC = actionB(&bufferHosts[i], allNodes[i].links[0].pkt);
                if (generateEventC){
                    insert(t, new_nodeS(C, i, 0L, currentTime +  defaultBias*33
                                    ));
                }
            }
            else if (ev->type == C)
            {
                ...
                if (generateEventB)
                    insert(t, new_nodeS(B, i, 0, currentTime + defaultBias*13));
                if (generateEventD){
                    insert(t, new_nodeS(D, nextIndex, nextPort, currentTime + loadingTime1));
                }
                #pragma endregion
            }
            else if (ev->type == D){
            ...
            else if (ev->type == E)
            {
                ...
                if (generateEventF){
                    insert(t, new_nodeS(F, i, portID, currentTime + SWITCH_CYCLE));
                }

                int idPrev = allNodes[i + numOfHosts].links[pickUpENB].nextIndex;
                idNodeInTree = 0;
                if (H_IS_HOST){
                    insert(t, new_nodeS(H_HOST, idPrev, 0, currentTime + 1));
                }
                else
                {
                    //generate event H
                    int idPrePort = allNodes[i + numOfHosts].links[pickUpENB].nextPort;
                    insert(t, new_nodeS(H, idPrev, idPrePort, currentTime + 1));
                }
                #pragma endregion
            }
            else if (ev->type == H_HOST)
            {
                ...
                if (generateEventC){
                    insert(t, new_nodeS(C, i, 0, currentTime + defaultBias * 33));
                }
            }
            else if (ev->type == F)
            {
                ...
                if (generateEventD_OR_G)
                {
                    ...
                    insert(t, new_nodeS(tempEvent, nextIndex, nextPort, currentTime + loadingTime));
                }
                #pragma endregion
            }
            else if (ev->type == G)
            {
                ...
                insert(t, new_nodeS(H, nextNode, nextPort, currentTime + 1));
                #pragma endregion
            }
            else if (ev->type == H)
            {
                ...
                if(generateEventF){
                    insert(t, new_nodeS(F, i, portID, currentTime));
                }
            }
